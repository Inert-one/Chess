window.onload = function(){
    ...
    chessContainer.classList.add("board");
    chessContainer.style.height = sw*8 + 'px';
    chessContainer.style.width = sw*8 + 'px';
    container.appendChild(chessContainer);
    chessContainer.style.top = 7+(h-tsw)/2+sw*(Math.floor(n/8)) + 'px';
    ...
    function updateSquarecolor(){
        for(var n = 0; n < 64; n++){
            if(Math.floor(n/8)%2 === 0){
                if(n%2 === 0){
                    sqs[n].style.background = 'rgba(255,255,255, 0.8)';
                }
                else {
                    sqs[n].style.background = 'rgba(196, 107, 43, 0)';
                }
            }
            else {
                if(n%2 === 1){
